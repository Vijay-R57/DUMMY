...
export class EvidenceFilterService {
  ...
  /**
   * Helper to expose the standalone threeStageMatch function statically on the class.
   */
  static threeStageMatch(
    description: string,
    canonical: string,
    aliases: Record<string, string[]>,
  ): boolean {
    return threeStageMatch(description, canonical, aliases);
  }
